...
@section('title', 'Viết bài mới - Tiệm Sách Nhà Gấu')
...
<div class="max-w-4xl mx-auto mt-8 bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-12 shadow-[0_8px_40px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_40px_rgb(0,0,0,0.2)] border border-gray-100 dark:border-gray-700">
    <div class="mb-10 text-center">
        <h1 class="text-3xl md:text-4xl font-serif font-bold text-brand-dark dark:text-gray-100 tracking-tight">Viết bài mới</h1>
        <p class="text-sm text-text-muted dark:text-gray-400 mt-2">Chia sẻ những dòng tản mạn hay review sách của bạn.</p>
    </div>
    ...
    <form action="{{ route('posts.store') }}" method="POST" class="space-y-8">
        ...
        <div class="group relative">
            ...
            <input type="text" name="title" id="title" class="w-full bg-brand-beige/30 dark:bg-gray-900/50 border-0 border-b-2 border-gray-200 dark:border-gray-700 focus:ring-0 focus:border-brand-brown px-0 py-3 text-2xl font-serif text-brand-dark dark:text-gray-100 transition-colors" placeholder="Một tựa đề thật thu hút..." value="{{ old('title') }}" required>
            @error('title')
        ...
        <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100 dark:border-gray-700">
            ...
            <button type="submit" class="px-8 py-3 bg-brand-brown text-white text-sm font-medium rounded-xl hover:bg-brand-dark transition-all shadow-md flex items-center gap-2 group">
                Lưu bài viết <span class="group-hover:translate-x-1 transition-transform">→</span>
            </button>
        </div>
    </form>
</div>
